 ...
 ######  list与vector区别？
   ...
   将元素遍历出来放入到list,然后通过list进行排序
    ...
    ######  TreeSet排序
        利用compartor进行排序；
    ######  HashSet与hashMap区别
    - hashSet不允许键值为空，hashmap允许key为空
    - hashset.add（）添加元素，hashMap.put()添加元素；
    - hashset是存储的是对象，hashmap存储键值对
    - hashset是利用对象来计算hash，hashmap利用键值进行hash，
    - hashmap较快于hashset，因为是通过键值查找。
    ...
    ######  什么情况下你会使用ArrayList？什么时候你会选择LinkedList？
     arrayList与linkedlist本质区别是数组和链表的两种数据结构的区别：ArrayList使用的内存量更大一些，在随机访问的效率高，删除，添加元素效率低。linkedlist刚好相反。
    ...
    ** 新生代几乎是所有的java对象出生的地方，java对象申请的内存和存放都是在这个地方。**
    ...
    pemGen space里面的内存空间就不会被释放，这样长时间的kill之后，就会导致系统的内存被渐渐吃完了，直到最后tomcat报错。
  ...
  **事务传播行为**
  ...
  - propagation_nested 表示如果当前已经存在一个事务了，那么就回嵌套在事务中运行。嵌套的事务可以独立于当前的事务进行提交或者回滚。如果当前事务不存在，那么他的行为和propagation_required一样
   ...
   ##### zookeeper工作原理
    ...
    - 抽象类比接口速度更快，接口是稍微会有点慢，因为它需要时间去寻找在各种实现的方法。
...
　　
    ...
    【 集合 collection常用的方法 add,remove ,contains,iterator,size:
     ...
     】
     - arrayList（list） 底层的数据结构是数组，查询快 ，增删慢；  线程不安全 ，效率高；
     - vector(list)  层次结构是数组，查询快，增删慢； 线程安全，效率低；
    ...
    ######  **堆也被称为优先队列，因为他的操作也是先进先出，从队尾插入原始，在队头取出元素。
 ...
 ##### hession
   ...
   ，如果重写进行你需要序列化的变量。
     ...
     **java的内存管理就是对象的分配和释放问题**
     ...
     1.函数基本类型变量和对象的引用变量都是在函数的栈内存中分配，线程也是在栈内存里面；
     ...
     **方法区**
     ...
     有一些可能是在运行时也将常量放入到常量池中，例如说string类有个native方法intern
     ...
     ###### jvm 检测工具使用
      jprofiler工具用于检测和跟踪系统（限于java开发）的性能。jprofiler可以通过实时监控系统内存使用情况，随时监视垃圾回收，线程运行状况等，可以
      很好的监控jvm运行情况和性能。
     ...
     在栈里面的内存不是被GC进行回收，GC只能清除堆上面分配的内存。那么在本地方法栈和栈里面的分配的内存清除就要
     ...
     ###### 内存泄漏
     是指你向系统申请的内存进行使用（new）,可是你使用以后不归还（delete），结果你申请的那块内存你自己不能再访问
